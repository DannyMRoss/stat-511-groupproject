...
\end{align}
where \texttt{treat} is the binary treatment indicator (job training program), and we allow for interactions between treatment and the pre-treatment covariates. We use automatic lambda selection (\texttt{search.lambdas = TRUE}) to choose optimal tuning parameters by the GCV algorithm, which selected $\lambda_Z = -4.9175$ and $\lambda_V = -2.9$. 
